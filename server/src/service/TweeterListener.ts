...
export class TweeterListener {
    ...
    static getInstance(_options: TwitterOptions) {
        if (this.tweeterListener == undefined) {
            this.tweeterListener = new TweeterListener(_options);
        }
        return this.tweeterListener;
    }

    constructor(_options: TwitterOptions) {
        ...
        const twitOptions: Twit.Options = <Twit.Options>{};
            ...
            try {
                if (this.options.listenTo) {
                    this.lookupUsers(this.options.listenTo).then((userIds) => {
                        this.options.userIds = <string>userIds;
                        resolve();
                    }, (err) => {
            ...
            try {
                ...
                this.twitterClient.get("users/lookup", twitParams)
                    .catch((err) => {
                        ...
                        const promiseRespnose: Twit.PromiseResponse = <Twit.PromiseResponse>result;
                        if (!(promiseRespnose.resp.statusCode == 200)) {
                            return reject("Error while getting user ids");
                        }
                        const twitterResponses: TwitterResponse[] = <TwitterResponse[]>promiseRespnose.data;
