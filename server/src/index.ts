...
import * as cloudantCfg from "./data/cloudant.config.json";
...
class App {
  ...
  constructor() {
    this.app = express();
    this.cloudantInitializer = new CloudantInitializer(config.cloudant_username, config.cloudant_password, cloudantCfg);
    this.config();
    this.routes();
    this.databaseSetup();
